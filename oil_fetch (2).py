...
def calc_sar(high, low, step=0.03, max_af=0.25):
    """Restituisce (sar_values, bull_state) — bull_state[i] = True/False/None"""
    n=len(high); sar=[None]*n; bull_arr=[None]*n
    if n<5: return sar, bull_arr
...
def calc_rsi_cross(rsi5, rsi14, dates):
    """Incroci RSI5/RSI14 — dir=1 rialzista (RSI5 sopra RSI14), dir=-1 ribassista"""
    crosses=[]; prev_diff=None
    for i in range(len(rsi5)):
        if rsi5[i] is None or rsi14[i] is None: continue
        diff = rsi5[i]-rsi14[i]
        if prev_diff is not None and prev_diff!=0 and (diff>0)!=(prev_diff>0):
            crosses.append({'idx': i, 'd': dates[i], 'dir': 1 if diff>0 else -1})
        prev_diff = diff
    last_cross = crosses[-1] if crosses else None
    return crosses, last_cross

def calc_accel(sar_bull, last_cross, dates):
    """Stato di accelerazione RSI5/RSI14 + SAR — SOLO VISIVO, non pesa su score/rating.
    PRE_SIGNAL: il cross RSI ha già dato una direzione ma il SAR non l'ha ancora confermata.
    CONFERMA:   il SAR ha flippato nella stessa direzione del cross (con N barre di ritardo)."""
    if not last_cross: return None
    n = len(sar_bull)
    sar_now = sar_bull[-1]
    if sar_now is None: return None
    # trova l'indice dell'ultimo flip SAR
    flip_idx = 0
    for i in range(2, n):
        if sar_bull[i] is not None and sar_bull[i-1] is not None and sar_bull[i]!=sar_bull[i-1]:
            flip_idx = i
    cross_is_bull = last_cross['dir']==1
    if cross_is_bull == sar_now:
        gap = max(0, flip_idx - last_cross['idx'])
        return {'stato':'CONFERMA', 'direzione': last_cross['dir'], 'gap_barre': gap, 'data_cross': last_cross['d']}
    else:
        gap = max(0, (n-1) - last_cross['idx'])
        return {'stato':'PRE_SIGNAL', 'direzione': last_cross['dir'], 'gap_barre': gap, 'data_cross': last_cross['d']}
...
def build_etp_block(ticker, start):
    ...
    rsi_crosses, last_cross = calc_rsi_cross(rsi5, rsi14, dates)
    accel = calc_accel(sar_bull, last_cross, dates)
    # data e streak dell'ultimo flip SAR (per display, indipendente da accel)
    sar_flip_idx = 0
    for i in range(2, len(sar_bull)):
        if sar_bull[i] is not None and sar_bull[i-1] is not None and sar_bull[i]!=sar_bull[i-1]:
            sar_flip_idx = i
    sar_date = dates[sar_flip_idx] if sar_flip_idx else None
    sar_streak = (len(sar_bull)-1) - sar_flip_idx
    ...
    return {
        ...
        'sarBull': sar_bull[-1], 'sarDate': sar_date, 'sarStreak': sar_streak,
        'rsi_crosses': rsi_crosses, 'last_cross': last_cross, 'accel': accel,
