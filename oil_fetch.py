...
TICKERS = {
    'long':  '3OIL.MI',
    'short': '3OIS.L',   # Yahoo non ha dati per 3OIS.MI (Milano) — quotazione presa da Londra (LSE, USD).
                          # Giorgio opera sulla quotazione di Milano: prezzi/segnali qui sono di RIFERIMENTO,
                          # possono divergere leggermente per cambio EUR/USD e liquidità di sede.
}
DATA_NOTE_SHORT = ("⚠️ Dati calcolati sulla quotazione di Londra (3OIS.L, USD) perché Yahoo Finance non fornisce "
                    "storico per 3OIS.MI. Lo strumento negoziato su Borsa Italiana è lo stesso ETC, ma prezzo e "
                    "segnali qui mostrati sono di riferimento e possono divergere leggermente per cambio EUR/USD "
                    "e liquidità della sede di quotazione.")
WTI_TICKER = 'CL=F'
...
def build_etp_block(ticker, start, data_note=None):
...
def main():
    ...
    if exec_type in ('morning', 'close', 'manual'):
        ...
        oil_short = build_etp_block(TICKERS['short'], "2018-01-01", data_note=DATA_NOTE_SHORT)
